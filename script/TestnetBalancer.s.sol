...
contract TestnetBalancerScript is Script {
  ...
  address private constant couponToken = address(0x8af55036DaCB876054E505d5bb76B2f8340060d5); // Fake
    // USDC
  address private constant wstEthToken = address(0x92862aa1ccB2B399a6236A6C7165aC54F7cB7a91); // Fake
    // wstETH
  address private constant cbEthToken = address(0x1197766B82Eee9c2e57674E53F0D961590e43769); // Fake
    // cbETH
  ...
  uint256 private constant distributionPeriod = 7_776_000; // 3 months in seconds (90 days * 24
  ...
  function run() public {
    ...
    address balancerOracleAdapter = Utils.deploy(
      address(new BalancerOracleAdapter()),
      abi.encodeCall(BalancerOracleAdapter.initialize, (balancerPoolToken, 18, oracleFeeds, msg.sender))
    );
    ...
    OracleFeeds(oracleFeeds).setPriceFeed(
      cbEthToken, address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE), cbEthPriceFeed, 1 days
    );
    OracleFeeds(oracleFeeds).setPriceFeed(
      wstEthToken, address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE), ethPriceFeed, 1 days
    );
    OracleFeeds(oracleFeeds).setPriceFeed(
      address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE), address(0), ethPriceFeed, 1 days
    );
