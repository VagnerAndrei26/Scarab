...
contract Router is OracleReader {
  ...
  function swapRedeem(
    ...
    if (tokenType == Pool.TokenType.LEVERAGE) tokenToTransfer = address(Pool(_pool).lToken());
    else tokenToTransfer = address(Pool(_pool).bondToken());
    ...
    uint256 usdcAmount = (redeemAmount * ethPrice).normalizeAmount(
      oracleDecimals + IERC20(reserveToken).safeDecimals(), IERC20(USDC).safeDecimals()
    );

    if (minAmount > usdcAmount) revert MinAmount();
