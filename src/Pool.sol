...
contract Pool is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, OracleReader, Validator {
  ...
  function distribute() external whenNotPaused {
    ...
    address distributorAdapter = poolFactory.distributorAdapters(address(this));

    uint256 adapterAmountToDistribute = DistributorAdapter(distributorAdapter).getDistributionAmount();

    // Transfer coupon tokens to the distributor
    IERC20(couponToken).safeTransfer(distributor, couponAmountToDistribute - adapterAmountToDistribute);
    IERC20(couponToken).safeTransfer(distributorAdapter, adapterAmountToDistribute);

    // Update distributor with the amount to distribute
    Distributor(distributor).allocate(couponAmountToDistribute - adapterAmountToDistribute);

    emit Distributed(previousPeriod, couponAmountToDistribute, distributor, distributorAdapter);
  }
