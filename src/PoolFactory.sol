...
contract PoolFactory is Initializable, AccessControlUpgradeable, UUPSUpgradeable, PausableUpgradeable {
  ...
  address public distributorAdapterBeacon;
  /// @dev Mapping to store distributor addresses for each pool
  mapping(address => address) public distributors;
  /// @dev Mapping to store distributor adapters for each pool
  mapping(address => address) public distributorAdapters;
  ...
  function createPool(
    ...
    // Deploy DistributorAdapter contract
    distributorAdapters[pool] = deployer.deployDistributorAdapter(distributorAdapterBeacon, pool, address(this));
   ...
   */
  function setDistributorAdapterBeacon(address _distributorAdapterBeacon) external onlyRole(GOV_ROLE) {
    if (distributorAdapterBeacon != address(0)) revert AlreadySet();
    distributorAdapterBeacon = _distributorAdapterBeacon;
  }
