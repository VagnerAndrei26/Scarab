...
library LogExpMath {
  ...
  /// @notice This error is thrown when an exponent used in the exp function is not within an
  /// acceptable range.
  error InvalidExponent();

  /// @notice This error is thrown when a variable or result is not within the acceptable bounds
  /// defined in the function.
  error OutOfBounds();

  // All fixed point multiplications and divisions are inlined. This means we need to divide by ONE
  // when multiplying
  ...
  // Bounds for ln_36's argument. Both ln(0.9) and ln(1.1) can be represented with 36 decimal places
  // in a fixed point
  // 256 bit integer.
  ...
  int256 constant a0 = 38_877_084_059_945_950_922_200_000_000_000_000_000_000_000_000_000_000_000; // eˆ(x0)
    // (no decimals)
  ...
  function pow(uint256 x, uint256 y) internal pure returns (uint256) {
    ...
    // range.
    if (x >> 255 != 0) revert BaseOutOfBounds();
    ...
    // This prevents y * ln(x) from overflowing, and at the same time guarantees y fits in the
    // signed 256 bit range.
    if (y >= MILD_EXPONENT_BOUND) revert ExponentOutOfBounds();
    ...
    unchecked {
      if (LN_36_LOWER_BOUND < x_int256 && x_int256 < LN_36_UPPER_BOUND) {
        ...
        // we can't just
        // bring y_int256 to 36 decimal places, as it might overflow. Instead, we perform two 18
        // decimal
        // multiplications and add the results: one with the first 18 decimals of ln_36_x, and one
        // with the
    ...
    if (!(MIN_NATURAL_EXPONENT <= logx_times_y && logx_times_y <= MAX_NATURAL_EXPONENT)) revert ProductOutOfBounds();
  ...
  function exp(int256 x) internal pure returns (int256) {
    if (!(x >= MIN_NATURAL_EXPONENT && x <= MAX_NATURAL_EXPONENT)) revert InvalidExponent();
    ...
    if (x < 0) {
      ...
      // positive since it
      // fits in the signed 256 bit range (as it is larger than MIN_NATURAL_EXPONENT). In the
      // negative
    ...
    // which we call x_n,
    // where x_n == 2^(7 - n), and e^x_n = a_n has been precomputed. We choose the first x_n, x0, to
    // equal 2^7
    // because all larger powers are larger than MAX_NATURAL_EXPONENT, and therefore not present in
    // the
    // decomposition.
    ...
    // `product` is the accumulated product of all a_n (except a0 and a1), which starts at 20
    // decimal fixed point
    ...
    unchecked {
      ...
      // Each term (x^n / n!) equals the previous one times x, divided by n. Since x is a fixed
      // point number,
      // multiplying by it requires dividing by ONE_20, but dividing by the non-fixed point n values
      // does not.
      ...
      // the Taylor
      // approximation of the exponentiation of the remainder (both with 20 decimals). All that
      // remains is to multiply
  ...
  function log(int256 arg, int256 base) internal pure returns (int256) {
    ...
    int256 logBase;
    unchecked {
      if (LN_36_LOWER_BOUND < base && base < LN_36_UPPER_BOUND) logBase = _ln_36(base);
      else logBase = _ln(base) * ONE_18;
    }

    int256 logArg;
    unchecked {
      if (LN_36_LOWER_BOUND < arg && arg < LN_36_UPPER_BOUND) logArg = _ln_36(arg);
      else logArg = _ln(arg) * ONE_18;
  ...
  function _ln(int256 a) private pure returns (int256) {
    ...
    if (a < ONE_18) {
      // Since ln(a^k) = k * ln(a), we can compute ln(a) as ln(a) = ln((1/a)^(-1)) = - ln((1/a)). If
      // a is less
      // than one, 1/a will be greater than one, so in this case we compute ln(1/a) and negate the
      // final result.
      unchecked {
        a = (ONE_18 * ONE_18) / a;
      }
    ...
    // First, we use the fact that ln^(a * b) = ln(a) + ln(b) to decompose ln(a) into a sum of
    // powers of two, which
    // we call x_n, where x_n == 2^(7 - n), which are the natural logarithm of precomputed
    // quantities a_n (that is,
    // ln(a_n) = x_n). We choose the first x_n, x0, to equal 2^7 because the exponential of all
    // larger powers cannot
    // be represented as 18 fixed point decimal numbers in 256 bits, and are therefore larger than
    // a.
    // At the end of this process we will have the sum of all x_n = ln(a_n) that apply, and the
    // remainder of this
    ...
    // For reasons related to how `exp` works, the first two a_n (e^(2^7) and e^(2^6)) are not
    // stored as fixed point
    // numbers with 18 decimals, but instead as plain integers with 0 decimals, so we need to
    // multiply them by
    // ONE_18 to convert them to fixed point.
    // For each a_n, we test if that term is present in the decomposition (if a is larger than it),
    // and if so divide
    ...
    unchecked {
      ...
      // All other a_n and x_n are stored as 20 digit fixed point numbers, so we convert the sum and
      // a to this format.
      sum *= 100;
      a *= 100;

      // Because further a_n are  20 digit fixed point numbers, we multiply by ONE_20 when dividing
      // by them.
    ...
    unchecked {
      ...
      // We now have the sum of all x_n present, and the Taylor approximation of the logarithm of
      // the remainder (both
      // with 20 decimals). All that remains is to sum these two, and then drop two digits to return
      // a 18 decimal
      // value.
  ...
  /**
   * @dev Internal high precision (36 decimal places) natural logarithm (ln(x)) with signed 18
   * decimal fixed point argument,
   * for x close to one.
   *
  ...
  function _ln_36(int256 x) private pure returns (int256) {
    ...
    unchecked {
      ...
      // We will use the following Taylor expansion, which converges very rapidly. Let z = (x - 1) /
      // (x + 1).
