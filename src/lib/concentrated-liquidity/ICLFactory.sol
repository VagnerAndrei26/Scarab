...
interface ICLFactory {
  ...
  /// A tick spacing can never be removed, so this value should be hard coded or cached in the
  /// calling context
  ...
  /// @notice Returns a list of enabled tick spacings. Used to iterate through pools created by the
  /// factory
  ...
  /// @notice Returns the pool address for a given pair of tokens and a tick spacing, or address 0
  ...
  /// @dev tokenA and tokenB may be passed in either order: token0/token1 or token1/token0. The call
  /// will
  /// revert if the pool already exists, the tick spacing is invalid, or the token arguments are
  /// invalid
