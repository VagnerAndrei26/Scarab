...
contract PoolTest is Test, TestCases {
  ...
  function testCreateDeadlineExactSuccess() public {
    ...
    // Mint reserve tokens
    rToken.mint(governance, 10_000_001_000);
  ...
  function testCreateDeadlineSimulateBlockAdvanceRevert() public {
    ...
    // Call create and assert minted tokens
    vm.expectRevert(Validator.TransactionTooOld.selector);
    _pool.create(Pool.TokenType.BOND, 1000, 30_000, block.timestamp, governance);
  }
  ...
  function testDistributeasd() public {
    ...
    uint256 expectedDistribution =
      (initialBalance + 10_000) * params.sharesPerToken / 10 ** _pool.bondToken().SHARES_DECIMALS();
    ...
    vm.mockCall(address(0), abi.encodeWithSignature("totalBuyCouponAmount()"), abi.encode(expectedDistribution));
  ...
  function testDistributeFailedPoolSale() public {
    ...
    vm.mockCall(address(0), abi.encodeWithSignature("state()"), abi.encode(uint256(3)));

    // Fast forward 5 days
    vm.warp(block.timestamp + 5 days);

    vm.mockCall(address(0), abi.encodeWithSignature("totalBuyCouponAmount()"), abi.encode(uint256(0)));
  ...
  function testDistributeFailedUndersold() public {
    ...
    // Force auction to fail during pool sale
    vm.mockCall(address(0), abi.encodeWithSignature("state()"), abi.encode(uint256(2)));
  ...
  function testClaimFees() public {
    ...
    params.fee = 20_000; // 2% fee (1000000 precision)
    ...
    uint256 expectedFee = (1000 ether * 20) / 1000; // 2% of 1000 ether
  ...
  function testCreateRedeemWithFees() public {
    ...
    // Calculate expected fee
    uint256 expectedFee = (100 ether * params.fee * 30 days) / (1_000_000 * 365 days);
  ...
  function testOracleInvertedPrice() public {
    vm.startPrank(deployer);
    mockPriceFeed.setMockPrice(27_887_401_483_629_120_000, 18);
    vm.stopPrank();
    ...
    assertEq(price, 35_858_486_155_012_863);

    uint256 price2 = _pool.getOraclePrice(params.reserveToken, _pool.USD());
    assertEq(price2, 27_887_401_483_629_120_000);
  }
  ...
  function testDoubleDistributionWithPeriodChange() public {
    ...
    uint256 initialUserBalance = 1000;

    vm.startPrank(address(_pool));
    _pool.bondToken().mint(user, initialUserBalance);
    vm.stopPrank();

    uint256 amountPerDistribution =
      (initialUserBalance + 10_000) * params.sharesPerToken / 10 ** _pool.bondToken().SHARES_DECIMALS();

    vm.startPrank(minter);
    couponToken.mint(address(_pool), amountPerDistribution);
    vm.stopPrank();
    ...
    vm.mockCall(address(0), abi.encodeWithSignature("totalBuyCouponAmount()"), abi.encode(amountPerDistribution));
    ...
    uint256 expectedClaimAmount =
      initialUserBalance * 2 * params.sharesPerToken / 10 ** _pool.bondToken().SHARES_DECIMALS();
    uint256 currentPeriod = 2;
    vm.expectEmit(true, true, true, true);
    emit Distributor.ClaimedShares(user, currentPeriod, expectedClaimAmount);

    distributor.claim();
    assertEq(couponToken.balanceOf(user), expectedClaimAmount);
    vm.stopPrank();
  }
}
