...
contract BalancerRouterTest is Test {
  ...
  function setUp() public {
    ...
    _pool = Pool(poolFactory.createPool(params, 100 ether, 10_000 * 10 ** 18, 10_000 * 10 ** 18, "", "", "", "", false));
    vm.stopPrank();
  ...
  function testJoinBalancerAndPlaza() public {
    ...
    assertEq(plazaTokens, 125_000_000_000_000_000_000, "Incorrect Plaza tokens received");
  ...
  function testExitPlazaAndBalancer() public {
    ...
    router.exitPlazaAndBalancer(
      BALANCER_POOL_ID, address(_pool), assets, plazaTokens, minAmountsOut, userData, Pool.TokenType.BOND, 0.9 ether
    );

    // Verify balances after exit
    assertEq(asset1.balanceOf(user), asset1BalanceBefore + 1 ether, "Incorrect asset1 balance after exit");
    assertEq(asset2.balanceOf(user), asset2BalanceBefore + 1 ether, "Incorrect asset2 balance after exit");
    assertEq(
      _pool.bondToken().balanceOf(user), bondTokenBalanceBefore - plazaTokens, "Incorrect bond token balance after exit"
    );
