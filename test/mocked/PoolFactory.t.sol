...
contract PoolFactoryTest is Test {
  ...
  function testCreatePool() public {
    ...
    emit PoolFactory.PoolCreated(address(0), 10_000_000_000, 10_000, 20_000);

    // Create pool and approve deposit amount
    Pool _pool = Pool(poolFactory.createPool(params, 10_000_000_000, 10_000, 20_000, "", "", "", "", false));
    uint256 endLength = poolFactory.poolsLength();

    assertEq(1, endLength - startLength);
    assertEq(rToken.totalSupply(), 10_000_000_000);
    assertEq(_pool.bondToken().totalSupply(), 10_000);
    assertEq(_pool.lToken().totalSupply(), 20_000);
  ...
  function testCreatePoolDeterministic() public {
    ...
    // address poolAddress = poolFactory.getPoolAddress(params.reserveToken, params.couponToken,
    // "bondWETH", "levWETH");

    bytes32 salt = keccak256(abi.encodePacked(params.reserveToken, params.couponToken, "bondWETH", "levWETH"));

    address proxyAddress = address(
      uint160(
        uint256(
          keccak256(abi.encodePacked(hex"ff", address(poolFactory), salt, Create3.KECCAK256_PROXY_CHILD_BYTECODE))
        )
      )
    );

    address poolAddress = address(uint160(uint256(keccak256(abi.encodePacked(hex"d694", proxyAddress, hex"01")))));
  ...
  function testCreatePoolErrors() public {
    ...
    poolFactory.createPool(params, 0, 10_000, 20_000, "", "", "", "", false);

    vm.expectRevert(bytes4(keccak256("ZeroDebtAmount()")));
    poolFactory.createPool(params, 10_000_000_000, 0, 20_000, "", "", "", "", false);

    vm.expectRevert(bytes4(keccak256("ZeroLeverageAmount()")));
    poolFactory.createPool(params, 10_000_000_000, 10_000, 0, "", "", "", "", false);

    vm.expectRevert(
      abi.encodeWithSelector(IERC20Errors.ERC20InsufficientAllowance.selector, address(poolFactory), 0, 10_000_000_000)
    );
    poolFactory.createPool(params, 10_000_000_000, 10_000, 10_000, "", "", "", "", false);
  }
